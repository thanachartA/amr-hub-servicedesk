...
export async function POST(req){
  ...
  let wb;
  try{ const buf = Buffer.from(String(file).replace(/^data:[^;]+;base64,/,""), "base64"); wb = XLSX.read(buf, { type:"buffer", cellDates:true }); }
  ...
  let projectId;
  if(existing){ projectId=existing.id; const { error:ue } = await admin.from("projects").update(projPatch).eq("id", projectId); if(ue) return Response.json({ error:"อัปเดตโครงการไม่สำเร็จ: "+ue.message }, { status:500 }); }
