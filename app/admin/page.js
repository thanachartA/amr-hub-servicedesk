...
export default function Admin(){
  ...
  useEffect(()=>{ (async()=>{
    ...
    setCanManage(["owner","lead"].includes(t?.hub_role)); setReady(true); load();
  })(); },[]);
  ...
  if(ready && !canManage) return <Shell title="จัดการผู้ใช้"><div className="card"><div className="muted">หน้านี้เฉพาะ Owner / Lead เท่านั้น</div></div></Shell>;
  ...
  return (<Shell title="จัดการผู้ใช้ & สิทธิ์">
    ...
    <div className="kpis" style={{gridTemplateColumns:"repeat(4,1fr)"}}>
      ...
      <div className="kpi red"><div className="n">{Object.values(team).filter(r=>r==="lead"||r==="owner").length}</div><div className="l">Owner / Lead</div></div>
      <div className="kpi"><div className="n">{Object.values(team).filter(r=>r==="supervisor").length}</div><div className="l">Supervisor</div></div>
    </div>
    <div className="card">
      ...
      <tbody>{shown.map(r=>{ const role=team[r.id]||"none";
        return (<tr key={r.id}>
          ...
          <td><select value={role} onChange={e=>setRole(r.id,e.target.value)} style={{minWidth:230,borderColor:border(role)}}>
            ...
            <option value="supervisor">Supervisor — ดู workload ทุกคน</option>
            <option value="lead">Lead — มอบหมาย + จัดการ</option>
            <option value="owner">Owner — สิทธิ์สูงสุด</option>
          </select></td>
        </tr>); })}
        {!shown.length&&<tr><td colSpan="4" className="muted">ไม่พบผู้ใช้</td></tr>}</tbody></table>
      <p className="muted" style={{marginTop:10}}><b>Owner</b> = ทุก module · <b>Lead</b> = มอบหมาย+จัดการ+เห็นทั้งหมด · <b>Supervisor</b> = ดู workload/รายงานทุกคน (ไม่มอบหมาย) · <b>Agent</b> = เห็นเฉพาะงานที่ได้รับ · <b>ผู้ขอ</b> = เปิดคำขอ+ดูของตัวเอง</p>
    </div>
  </Shell>);
}
