...
export default function Dashboard(){
  const [k,setK]=useState({open:0,breach:0,doneWk:0,onSla:0}); const [rows,setRows]=useState([]);
  useEffect(()=>{ (async()=>{
    ...
    const open=list.filter(r=>!["closed","cancelled","done"].includes(r.status)).length;
    const breach=list.filter(r=>r.sla_due_at&&new Date(r.sla_due_at)<now&&!["closed","cancelled","done"].includes(r.status)).length;
    ...
    setK({open,breach,doneWk,onSla}); setRows(list.slice(0,8));
  })(); },[]);
