...
import { brandFor } from "../../../lib/brandLogos";
...
export default function RequestDetail(){
  ...
  function printTravelPDF(){
    ...
    const brand=brandFor(r.project?.code);
    ...
    const totToll=list.reduce((s,t)=>s+(Number(t.toll)||0),0);
    const esc=(s)=>String(s==null?"":s).replace(/[&<>]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;"}[m]));
    const rows=list.map(t=>`<tr><td style="text-align:center">${t.no}</td><td>${esc(t.date)}</td><td>${esc(t.dest)}</td><td style="text-align:right">${t.odo_out==null?"":t.odo_out}</td><td style="text-align:right">${t.odo_in==null?"":t.odo_in}</td><td style="text-align:right">${t.km==null?"":t.km}</td><td style="text-align:right">${t.maps_km==null?"":t.maps_km}</td><td style="text-align:center">${t.over?"⚠ ตรวจสอบ":"✓ ปกติ"}</td><td style="text-align:right">${(Number(t.toll)||0)?fmtMoney(t.toll):""}</td><td style="text-align:right">${fmtMoney(t.amount)}</td></tr>`+((t.over&&t.reason)?`<tr><td></td><td colspan="9" style="color:#b03a2e;font-size:11px">เหตุผล/จุดแวะ: ${esc(t.reason)}</td></tr>`:"")).join("");
...
<div style="text-align:center;margin-bottom:4px"><img src="${brand.logo}" style="height:42px"/></div>
<div style="font-size:11px;color:#555;text-align:center">${esc(brand.name)} · เลขที่คำขอ ${esc(r.ticket_no)} · ตามประกาศ AMR14/2569</div>
      ...
      <div>
        ...
        {trips&&(<div className="card">
          ...
          <table style={{fontSize:12}}><thead><tr>
            <th>#</th><th>วันที่</th><th>ปลายทาง</th><th className="right">ไมล์ไป</th><th className="right">ไมล์กลับ</th><th className="right">กม.</th><th className="right">Maps</th><th>ตรวจ</th><th className="right">ทางด่วน</th><th className="right">เงิน</th>
          </tr></thead><tbody>
          {trips.map(t=>(<Fragment key={t.no}>
            <tr>
              ...
              <td className="right">{t.toll?fmtMoney(t.toll):"-"}</td>
              <td className="right">{fmtMoney(t.amount)}</td>
            </tr>
            {t.over&&t.reason&&<tr><td></td><td colSpan="9" style={{fontSize:11,color:"#B03A2E"}}>เหตุผล/จุดแวะ: {t.reason}</td></tr>}
          </Fragment>))}
          <tr style={{fontWeight:700,borderTop:"2px solid #DDE6E0",background:"#FAFDFB"}}>
            ...
            <td className="right">{r.form_data?.tolls_total?fmtMoney(r.form_data.tolls_total):"-"}</td>
